## Mean buffer fPAR as a function of time
...
fparDf$BUFF25_FPAR_MEAN[fparDf$BUFF25_FPAR_MEAN == 0] <- 0.000001

## Exploratory analyses ---- 

# Check distribution of response variable
hist(fparDf$BUFF25_FPAR_MEAN)
# These data fall between 0 and 1, suggesting best distribution is the beta family

# Check for outliers
dotchart(fparDf$BUFF25_FPAR_MEAN, xlab = "Buffer fPAR", ylab = "Order of the data")
# No outliers

# Test fit of gaussian vs. beta distribution
m0 <- glmmTMB(BUFF25_FPAR_MEAN ~ YEAR_s*IUCN_GROUP, 
             data = fparDf,
             REML = TRUE)

# Compare gaussian model above to a tweedie model
m1 <- glmmTMB(BUFF25_FPAR_MEAN ~ YEAR_s*IUCN_GROUP, 
              family = beta_family(),  
              data = fparDf,
              REML = TRUE)
...
m <- glmmTMB(BUFF25_FPAR_MEAN ~ YEAR_s*IUCN_GROUP, 
             family = beta_family(),  
             data = fparDf,
             REML = TRUE)

m.b <- glmmTMB(BUFF25_FPAR_MEAN ~ YEAR_s*IUCN_GROUP + (1|BIOME25), 
               family = beta_family(),  
               data = fparDf,
               REML = TRUE)

m.c <- glmmTMB(BUFF25_FPAR_MEAN ~ YEAR_s*IUCN_GROUP + (1|CONTINENT), 
               family = beta_family(),  
               data = fparDf,
               REML = TRUE)

m.w <- glmmTMB(BUFF25_FPAR_MEAN ~ YEAR_s*IUCN_GROUP + (1|WDPA_PID), 
               family = beta_family(),  
               data = fparDf,
               REML = TRUE)

m.bw <- glmmTMB(BUFF25_FPAR_MEAN ~ YEAR_s*IUCN_GROUP + (1|BIOME25) + (1|WDPA_PID), 
                family = beta_family(),  
                data = fparDf,
                REML = TRUE)

m.cw <- glmmTMB(BUFF25_FPAR_MEAN ~ YEAR_s*IUCN_GROUP + (1|CONTINENT) + (1|WDPA_PID), 
                family = beta_family(),  
                data = fparDf,
                REML = TRUE)

m.bc <- glmmTMB(BUFF25_FPAR_MEAN ~ YEAR_s*IUCN_GROUP + (1|BIOME25) + (1|CONTINENT), 
                family = beta_family(),  
                data = fparDf,
                REML = TRUE)

m.all <- glmmTMB(BUFF25_FPAR_MEAN ~ YEAR_s*IUCN_GROUP + (1|BIOME25) + (1|CONTINENT) + (1|WDPA_PID), 
                 family = beta_family(),  
                 data = fparDf,
                 REML = TRUE)
...
## Test random slopes ----
m.all <- glmmTMB(BUFF25_FPAR_MEAN ~ YEAR_s*IUCN_GROUP + (1+YEAR_s|BIOME25) + (1+YEAR_s|CONTINENT) + (1|WDPA_PID), 
                 family = beta_family(),  
                 data = fparDf,
                 REML = TRUE)

m.bio <- glmmTMB(BUFF25_FPAR_MEAN ~ YEAR_s*IUCN_GROUP + (1+YEAR_s|BIOME25) + (1|CONTINENT) + (1|WDPA_PID), 
                 family = beta_family(),  
                 data = fparDf,
                 REML = TRUE,
                 control=glmmTMBControl(optimizer=optim,optArgs=list(method="BFGS")))

m.con <- glmmTMB(BUFF25_FPAR_MEAN ~ YEAR_s*IUCN_GROUP + (1|BIOME25) + (1+YEAR_s|CONTINENT) + (1|WDPA_PID), 
                 family = beta_family(),  
                 data = fparDf,
                 REML = TRUE)
...
# Significant SAC was detected (observed = 3.9116e-01 ; p-value < 2.2e-16).
...
df03 <- fparDf[(fparDf$YEAR == 0),] # Get data for 2003
s1 <- df03[sample(nrow(df03), 3489),] # Take a random sample
...
m <- glmmTMB(BUFF25_FPAR_MEAN ~ YEAR_s*IUCN_GROUP + (1|BIOME25) + (1|CONTINENT) + (1|WDPA_PID), 
             family = beta_family(),  
             data = fparDfSubset,
             REML = TRUE)
...
m.g <- glmmTMB(BUFF25_FPAR_MEAN ~ YEAR_s*IUCN_GROUP + (1|BIOME25) + (1|CONTINENT) + (1|WDPA_PID) + gau(0 + pos|group), 
               family = beta_family(),  
               data = fparDfSubset,
               REML = TRUE)

# Fit spatial model - exponential
m.e <- glmmTMB(BUFF25_FPAR_MEAN ~ YEAR_s*IUCN_GROUP + (1|BIOME25) + (1|CONTINENT) + (1|WDPA_PID) + exp(0 + pos|group), 
               family = beta_family(),  
               data = fparDfSubset,
               REML = TRUE)
               #control=glmmTMBControl(optimizer=optim,optArgs=list(method="BFGS")))
...
# N.B: Did not converge
m.m <- glmmTMB(BUFF25_FPAR_MEAN ~ YEAR_s*IUCN_GROUP + (1|BIOME25) + (1|CONTINENT) + (1|WDPA_PID) + mat(0 + pos|group), 
               family = beta_family(),  
               data = fparDfSubset,
               REML = TRUE)
...
finalModel <- glmmTMB(BUFF25_FPAR_MEAN ~ YEAR_s*IUCN_GROUP + (1|BIOME25) + (1|CONTINENT) + (1|WDPA_PID) + exp(0 + pos|group), 
                      family = beta_family(),  
                      data = fparDfSubset,
                      REML = TRUE)
#control=glmmTMBControl(optimizer=optim,optArgs=list(method="BFGS")))
